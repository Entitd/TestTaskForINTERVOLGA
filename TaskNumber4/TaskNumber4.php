...
function truncateText($text, $length) {
    $matches = [];
    preg_match_all('/<i>(.+?)<\/i>/', $text, $matches);
    ...
    foreach ($matches[1] as $match) {
        ...
        $truncateString = implode(' ', array_splice($words, 0, $length)) . '...';
        $text = str_replace($rawString, $truncateString, $text);
    }

    return $text;
}

echo truncateText($text, 29);
?>